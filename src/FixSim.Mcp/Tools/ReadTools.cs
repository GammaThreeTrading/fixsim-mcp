...
public sealed class ReadTools
{
    ...
    public Task<JsonNode?> ListOrdersIn(string instance, string session, CancellationToken ct,
        [Description("Optional FIX OrdStatus (tag 39) filter")] string? orderStatus = null,
        string? apiKey = null)
        => _api.GetAsync(_key.Resolve(apiKey),
            $"v1/OrdersIn/{U(instance)}/{U(session)}" + (orderStatus is null ? "" : $"?orderstatus={U(orderStatus)}"), ct);
    ...
    public Task<JsonNode?> ListExecutionsOut(string instance, string session, CancellationToken ct,
        [Description("Optional ClOrdID (tag 11) to narrow to one order")] string? clOrdId = null,
        string? apiKey = null)
        => _api.GetAsync(_key.Resolve(apiKey),
            $"v1/ExecutionsOut/{U(instance)}/{U(session)}" + (clOrdId is null ? "" : $"/{U(clOrdId)}"), ct);
    ...
    public Task<JsonNode?> ListExecutionsIn(string instance, string session, CancellationToken ct,
        [Description("Optional ExecID (tag 17) filter")] string? execId = null,
        string? apiKey = null)
        => _api.GetAsync(_key.Resolve(apiKey),
            $"v1/ExecutionsIn/{U(instance)}/{U(session)}" + (execId is null ? "" : $"?execid={U(execId)}"), ct);
    ...
    public Task<JsonNode?> ListOrdersOut(string instance, string session, CancellationToken ct,
        string? clOrdId = null, string? apiKey = null)
        => _api.GetAsync(_key.Resolve(apiKey),
            $"v1/OrdersOut/{U(instance)}/{U(session)}" + (clOrdId is null ? "" : $"/{U(clOrdId)}"), ct);
